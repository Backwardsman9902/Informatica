...
final def groovyScript = PLUGIN_HOME + FS + "create_dynamic_deployment.groovy"
...
out.println([groovy, "-cp", classpath, groovyScript,args[0], args[1]].join(' '))

def process = [groovy, "-cp", classpath, groovyScript, args[0], args[1]].execute()
